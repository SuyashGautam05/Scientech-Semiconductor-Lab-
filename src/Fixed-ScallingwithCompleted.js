...
// Auto-scaling function with fixed ranges
function updateScale() {
  // Get the current axis selections
  const xAxis = xAxisSelect.value;
  const yAxis = yAxisSelect.value;
  
  // Set x-axis range based on selected axis
  if (xAxis === 'V1' || xAxis === 'V2') {
    // Voltage range: 0 to 20
  ...
  // Set y-axis range based on selected axis
  if (yAxis === 'V1' || yAxis === 'V2') {
    // Voltage range: 0 to 20
...
function recalculateScaling() {
  // We don't need to recalculate min/max as we're using fixed ranges
  // Just call updateScale to apply the fixed ranges
  updateScale();
}
...
ipcRenderer.on('serial-data', (event, data) => {
  ...
  const currentX = data[xAxis];
  const currentY = data[yAxis];
  
  // Check if we're going backward (decreasing X value)
  if (currentPoint) {
    const prevX = currentPoint[xAxis]; // Get X value from previous point using current axis
    if (currentX < prevX) {
      // Going backward - remove all points with X values greater than current X
      for (let i = dataPoints.length - 1; i >= 0; i--) {
        if (dataPoints[i][xAxis] > currentX) {
  ...
  // Update scaling with fixed ranges
  updateScale();
  
  // Update chart datasets - extract x,y from full data objects
  chart.data.datasets[0].data = dataPoints.map(point => ({
    x: point[xAxis],
    y: point[yAxis]
  }));
  chart.data.datasets[1].data = [{
    x: currentPoint[xAxis],
    y: currentPoint[yAxis]
  }];
